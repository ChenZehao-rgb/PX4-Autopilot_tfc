...
#include <uORB/topics/pressure_force_sensor.h>
...
class PressureForceSensor : public ModuleBase<PressureForceSensor>,
                    public ModuleParams,
                    public px4::ScheduledWorkItem
{
public:
    PressureForceSensor();
    ~PressureForceSensor() override;

    int init();

    static int task_spawn(int argc, char *argv[]);
    static PressureForceSensor *instantiate(int argc, char *argv[]);
...
private:
    ...
    // uORB 发布器
    uORB::Publication<pressure_force_sensor_s> _pub{ORB_ID(pressure_force_sensor)};
    uORB::Publication<debug_array_s> _debug_pub{ORB_ID(debug_array)};

    pressure_force_sensor_s msg{};
...
static PressureForceSensor *g_instance{nullptr};

/* ============================= 实现 ============================= */

PressureForceSensor::PressureForceSensor()
...
int PressureForceSensor::configure_port(int fd, speed_t baud)
{
    struct termios cfg{};
...
int PressureForceSensor::init()
{
    ...
    _reader_task = px4_task_spawn_cmd(
        ...
        2000,                       // 栈
        (px4_main_t)&PressureForceSensor::reader_trampoline,
        nullptr);
    ...
    PX4_INFO("pressure_force_sensor (blocking) dev=%s baud=%d, use_debug=%d", _device, _baudrate, _use_debug);
    return PX4_OK;
}

int PressureForceSensor::reader_trampoline(int, char**)
{
    return g_instance ? g_instance->reader_loop() : -1;
}

int PressureForceSensor::reader_loop()
{
    // 在子任务里打开串口（很关键）
...
int PressureForceSensor::task_spawn(int argc, char *argv[])
{
    // 默认参数
    ...
    PressureForceSensor *instance = new PressureForceSensor();
    if (!instance) {
...
PressureForceSensor *PressureForceSensor::instantiate(int, char*[])
{
    return nullptr; // 不使用
}

int PressureForceSensor::print_usage(const char *reason)
{
    if (reason) {
        PX4_ERR("%s", reason);
    }
    ...
    PRINT_MODULE_DESCRIPTION(
        R"DESC(
### Description
Reads frames from a serial device ('R' + 16 ASCII digits) and publishes four parsed values on `pressure_force_sensor`.
...
pressure_force_sensor stop
    ...
    PRINT_MODULE_USAGE_NAME("pressure_force_sensor", "examples");
...
int PressureForceSensor::custom_command(int, char*[])
{
    return print_usage("unrecognized command");
}

extern "C" __EXPORT int pressure_force_sensor_main(int argc, char *argv[])
{
    return PressureForceSensor::main(argc, argv);
}

// 可选别名，部分环境会用 *_app_main 入口
extern "C" __EXPORT int pressure_force_sensor_app_main(int argc, char *argv[])
{
    return pressure_force_sensor_main(argc, argv);
}
