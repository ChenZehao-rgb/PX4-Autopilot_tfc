/************************************************************************************
 * pressure_force_sensor.cpp (blocking reader for real device + ScheduledWorkItem for SIM)
 *
 * Features:
 *  - Real device: dedicated thread with poll()+read()  阻塞读，省 CPU、低延迟
 *  - SIM mode: ScheduledWorkItem 定时喂“R+16位数字”帧
 *  - 共用解析器 handle_bytes()，uORB 发布 pressure_force_sensor topic
 *
 * Usage:
 *   pressure_force_sensor start [-d <device>] [-b <baud>] [--sim [Hz]]
 *   pressure_force_sensor status
 *   pressure_force_sensor stop
 *
 ************************************************************************************/
...
#include <uORB/topics/pressure_force_sensor_ref.h>
...
class PressureForceSensorRef : public ModuleBase<PressureForceSensorRef>,
                    public ModuleParams,
                    public px4::ScheduledWorkItem
{
public:
    PressureForceSensorRef();
    ~PressureForceSensorRef() override;

    int init();

    static int task_spawn(int argc, char *argv[]);
    static PressureForceSensorRef *instantiate(int argc, char *argv[]);
...
private:
    ...
    // uORB 发布器
    uORB::Publication<pressure_force_sensor_ref_s> _pub{ORB_ID(pressure_force_sensor_ref)};

    pressure_force_sensor_ref_s msg{};
    // 帧缓冲
...
static PressureForceSensorRef *g_instance{nullptr};

/* ============================= 实现 ============================= */

PressureForceSensorRef::PressureForceSensorRef()
...
void PressureForceSensorRef::Run()
{
...
int PressureForceSensorRef::configure_port(int fd, speed_t baud)
{
    struct termios cfg{};
...
int PressureForceSensorRef::init()
{
    ...
    _reader_task = px4_task_spawn_cmd(
        ...
        2000,                       // 栈
        (px4_main_t)&PressureForceSensorRef::reader_trampoline,
        nullptr);
    ...
    PX4_INFO("pressure_force_sensor_ref (blocking) dev=%s baud=%d", _device, _baudrate);
    return PX4_OK;
}

int PressureForceSensorRef::reader_trampoline(int, char**)
{
    return g_instance ? g_instance->reader_loop() : -1;
}

int PressureForceSensorRef::reader_loop()
{
    // 在子任务里打开串口（很关键）
...
int PressureForceSensorRef::task_spawn(int argc, char *argv[])
{
    // 默认参数
    const char *device = "/dev/ttyS3"; //接UART4
    int baud = 115200;
    ...
    PressureForceSensorRef *instance = new PressureForceSensorRef();
    if (!instance) {
...
PressureForceSensorRef *PressureForceSensorRef::instantiate(int, char*[])
{
    return nullptr; // 不使用
}

int PressureForceSensorRef::print_usage(const char *reason)
{
    if (reason) {
        PX4_ERR("%s", reason);
    }
    ...
    PRINT_MODULE_DESCRIPTION(
        R"DESC(
### Description
Reads frames from a serial device ('R' + 16 ASCII digits) and publishes four parsed values on `pressure_force_sensor_ref`.
...
### Usage
pressure_force_sensor_ref start [-d <device>] [-b <baud>] [--sim [Hz]]
  -d, --device : serial path (default /dev/ttyS3)
  -b, --baud   : baudrate (default 115200)

pressure_force_sensor_ref status
pressure_force_sensor_ref stop
)DESC");

    // use example to print parameter info
    PRINT_MODULE_USAGE_NAME("pressure_force_sensor_ref", "examples");
...
int PressureForceSensorRef::custom_command(int, char*[])
{
    return print_usage("unrecognized command");
}

extern "C" __EXPORT int pressure_force_sensor_ref_main(int argc, char *argv[])
{
    return PressureForceSensorRef::main(argc, argv);
}

// 可选别名，部分环境会用 *_app_main 入口
extern "C" __EXPORT int pressure_force_sensor_ref_app_main(int argc, char *argv[])
{
    return pressure_force_sensor_ref_main(argc, argv);
}
